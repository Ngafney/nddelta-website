/**
 * The book. One row per tick. Week 2 ran it with no visible end, because the
 * settlement range was a secret; week 3 passes `lo`/`hi` (1 and 99) and the
 * ladder simply stops there, because everybody knows a probability does. Only
 * the slice on screen is ever in the DOM either way.
 *
...
export default function OrderBook({
  ...
  const [span, setSpan] = useState(() =>
    bounded ? { lo: Math.round((lo - center) / tick), hi: Math.round((hi - center) / tick) } : { lo: -SLAB, hi: SLAB }
  );
  ...
  const scrollToPx = useCallback(
    (px, smooth) => {
      ...
      if (!bounded) setSpan((s) => ({ lo: Math.min(s.lo, i - SLAB), hi: Math.max(s.hi, i + SLAB) }));
  ...
  const onScroll = (e) => {
    ...
    if (bounded || rows >= MAX_ROWS) return;
  ...
  return (
    <div>
      ...
      <div className="hint">
        {bounded
          ? `Prices run ${lo} to ${hi} — read them as a probability in percent. Zoom to fit more on screen, or jump back to the middle.`
          : "The ladder keeps going both ways — scroll as far as you like, zoom to fit more prices on screen, or jump back to the middle."}{" "}
        Clicking above the best offer, or below the best bid, crosses the book and trades at the
        resting price.
