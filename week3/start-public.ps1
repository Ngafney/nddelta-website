# Start Week 3 — Coin Flips — on a public URL from this machine.
#   Right-click -> Run with PowerShell, or: powershell -File week3\start-public.ps1
#
...
if (-not (Test-Path (Join-Path $week3 "..\public\week3\index.html"))) {
...
if (-not $up) {
  ...
  Start-Process -WindowStyle Hidden node -ArgumentList "serve.js" -WorkingDirectory $week3
...
Write-Host ""
Write-Host "  players -> <public-url>/week3/"
Write-Host "  admin   -> <public-url>/week3/admin     (password 123 until you change it)"
Write-Host "  screen  -> <public-url>/week3/board"
Write-Host ""

# 2. Cloudflare tunnel — prints a fresh https URL each run.
Write-Host "Opening a public tunnel (this window must stay open) ..."
Write-Host "Watch below for:  https://<something>.trycloudflare.com  - add /week3/ to it."
Write-Host ""
