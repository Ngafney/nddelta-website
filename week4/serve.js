/**
 * One server, one port: serves the built Week 4 app AND its API on the same
 * origin, bound to 0.0.0.0 so it can be tunneled / reached off-box.
 *   node serve.js            → http://0.0.0.0:8082/week4/
 *
 ...
 * Loads week4/.env (SESSION_SECRET, …) like dev-server.
 */
...
const STATIC_DIR = path.join(here, "..", "public", "week4"); // vite build output
...
http
  .createServer((req, res) => {
    ...
    if (pathname.startsWith("/api/week4/")) {
      const route = pathname.replace(/^\/api\/week4\//, "").replace(/\/$/, "");
      return nodeHandler(req, res, route);
    }
    if (pathname === "/" || pathname === "/week4") {
      res.statusCode = 302;
      res.setHeader("Location", "/week4/");
      return res.end();
    }
    if (pathname.startsWith("/week4/")) {
      const rel = pathname.slice("/week4/".length);
    ...
    res.statusCode = 404;
    res.end("not found — try /week4/");
  })
  .listen(PORT, "0.0.0.0", () => {
    console.log(`week4    → http://0.0.0.0:${PORT}/week4/`);
    console.log(`admin    → http://0.0.0.0:${PORT}/week4/admin`);
    console.log(`board    → http://0.0.0.0:${PORT}/week4/board`);
