...
export function fillToasts(fills) {
  ...
  for (const f of fills) {
    // The market is part of the key: the two books fill independently, and
    // one notification saying "bought 10" without saying WHICH is useless.
    const key = `${f.market ?? ""}${f.side}${f.taker ? "T" : "M"}`;
    const g =
      groups.get(key) ??
      { market: f.market ?? null, side: f.side, taker: f.taker, qty: 0, lo: f.px, hi: f.px, seq: f.s, names: new Set() };
    g.qty += f.qty;
