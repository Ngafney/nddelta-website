/**
 * Vercel entry for the whole Week 4 API. A vercel.json rewrite sends every
 * /api/week4/* request here — nested paths included — with the sub-path in the
 ...
 */
import { nodeHandler } from "../../week4/server/handler.js";

export default async function (req, res) {
  const url = new URL(req.url, "http://x");
  let route = url.searchParams.get("__path");
  if (route == null) route = url.pathname.replace(/^\/api\/week4\/?/, "");
